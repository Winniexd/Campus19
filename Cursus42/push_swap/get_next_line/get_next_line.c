...
/*   Updated: 2024/01/23 16:56:59 by matias           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_parse(char *start)
{
	...
	if (end != NULL)
		...
		len = gnl_strlen(start);
	line = (char *)malloc(len + 1);
	if (!line)
		return (NULL);
	gnl_memcpy(line, start, len);
	line[len] = '\0';
	return (line);
}
...
char	*norminette_is_dumb(int fd, char *start_line)
{
	...
	while (!(gnl_strchr(start_line, '\n')) && ret != 0)
	{
		...
		line[ret] = '\0';
		start_line = gnl_strjoin(start_line, line);
	}
	free(line);
	return (start_line);
}
